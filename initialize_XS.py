...
def rotcur(galaxy, vel_map2D,evel_map,SN,VSYS,PA,INC,X0,Y0,n_it =5, pixel_scale=1,vary_PA=True,vary_INC=True,vary_XC=True,vary_YC=True,vary_VSYS=True,vary_PHI=True,delta=1, rstart=2, rfinal=50, ring_space=2, frac_pixel=2/3., r_bar_max=20,vmode="circular", save_plots = 1, errors = 1,survey = "", config = "", e_ISM = 5 ):
	"""
	...
	def run_code():
		...
		if vmode == "bisymmetric":
			PA,INC,XC,YC,VSYS,THETA,R,Vrot,Vrad,Vtan,MODEL,PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r,errors_fit = init_models(vel_ha,evel_map,guess,vary, n_it, sigma, delta, rstart, rfinal, ring_space, frac_pixel,r_bar_max,pixel_scale,vmode,errors, config, e_ISM)
		else:

			PA,INC,XC,YC,VSYS,THETA,R,Vrot,Vrad,Vtan,MODEL,CHISQ_r,errors_fit = init_models(vel_ha,evel_map,guess,vary, n_it, sigma, delta, rstart, rfinal, ring_space, frac_pixel, r_bar_max,pixel_scale,vmode,errors, config, e_ISM)
			PA_BAR_MAJOR,PA_BAR_MINOR = 0,0

		return PA,INC,XC,YC,VSYS,THETA,R,Vrot,Vrad,Vtan,MODEL,PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r,errors_fit




	PA,INC,XC,YC,VSYS,THETA,R,Vrot,Vrad,Vtan,MODEL,PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r,errors_fit = run_code()


	#
	# MCMC to estimate errors on the derived parameters
	#

	#MCMC_results
	
	Vrot_MCMC,e_Vrot,Vrad_MCMC,e_Vrad,PA_MCMC,e_PA,INC_MCMC,e_INC,X0_MCMC,XC_e,YC_MCMC,YC_e,Vsys_MCMC,e_Vsys,theta,e_theta,Vtan_MCMCM,e_Vtan = errors_fit	
	...
	if vmode == "circular" or vmode == "radial":
		...
		if errors == 0:
			...
			#kin_params = [galaxy,XC,0,0,YC,0,0,PA,0,0,INC,0,0,VSYS,0,0,CHISQ_r]
			kin_params = [galaxy,XC,XC_e[0],XC_e[1],YC,YC_e[0],YC_e[1],PA,e_PA[0],e_PA[1],INC,e_INC[0],e_INC[1],VSYS,e_Vsys[0],e_Vsys[1],CHISQ_r]
	...
	if vmode == "bisymmetric":
		...
		if errors == 0:
			...
			#kin_params = [galaxy,XC,0,0,YC,0,0,PA,0,0,INC,0,0,VSYS,0,0,THETA,0,0,PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r]
			kin_params = [galaxy,XC,XC_e[0],XC_e[1],YC,YC_e[0],YC_e[1],PA,e_PA[0],e_PA[1],INC,e_INC[0],e_INC[1],VSYS,e_Vsys[0],e_Vsys[1],THETA,e_theta[0],e_theta[1],PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r]
	...
	plot_kin_models(galaxy,vmode,vel_ha,R,Vrot,e_Vrot,Vrad,e_Vrad,Vtan,e_Vtan, VSYS, MODEL2D, ext,plot = 0, save = save_file)
	...
	save_vlos_model(galaxy,vmode,vel_map,evel_map,MODEL2D,PA,INC,XC,YC,VSYS,save = save_file)
