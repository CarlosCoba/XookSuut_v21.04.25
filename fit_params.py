...
#####
		...
		from create_2D_model import best_2d_model
		V_k = [Vrot, 0, 0] 
		best_model = best_2d_model(vmode, shape, V_k, pa, inc, x0, y0, Vsys, rings_pos, ring_space = ring_space, pixel_scale = pixel_scale) 
		...
		return best_model,Vrot, Vsys, pa, inc , abs(x0), abs(y0) ,red_chi, N_free, Std_errors
	...
	if vmode == "radial":
		...
		return best_model, Vrot, Vrad, Vsys, pa, inc , abs(x0), abs(y0) ,red_chi, N_free, Std_errors
	...
	if vmode == "bisymmetric":
		...
		from create_2D_model import best_2d_model
		V_k = [Vrot, Vrad, Vtan] 
		best_model = best_2d_model(vmode, shape,V_k, pa, inc, x0, y0, Vsys, rings_pos, ring_space = ring_space, phi_b = theta, pixel_scale = pixel_scale) 

	
		Std_errors = [std_Vrot,std_Vrad,std_pa, std_inc, std_x0, std_y0, std_Vsys, std_theta, std_Vtan]


		return best_model, Vrot, Vrad, Vsys,  pa, inc , x0, y0, Vtan, theta, red_chi, N_free, Std_errors
